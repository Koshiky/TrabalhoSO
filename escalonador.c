...
//Rotina dos filhos
    if((pid = fork()) == 0){
      while (1) {
        //Caso fila vazia, wait para lidar com processo zombie, passando pelas filas de prioridades
        for(i=0; i < 3; i++){
            if (msgrcv(idfila, &msgrec, sizeof(struct mensagem), i+1, IPC_NOWAIT) == -1) {
              waitpid(-1, &estado, WNOHANG);
                ...
                for(j=0;j<msgrec.exec.n;j++ )
                    if((pid = fork()) == 0) {
                      if((pid = fork()) == 0 ){
                        execl(msgrec.exec.name, "", (char*) NULL);
                      }
          ...
          if (msgrcv(idfila, &msgrec, sizeof(struct mensagem), i+1, IPC_NOWAIT) == -1) {
            ...
            *nProc = *nProc + 1;
            ...
            if((pid = fork()) == 0){
                ...
                printf("Job %d - .exe %s sendo repassado ao escalonador\n", msgsen.exec.job, msgsen.exec.name);
